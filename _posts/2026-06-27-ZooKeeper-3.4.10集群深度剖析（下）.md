...
#### 9.6.7 特殊场景深度分析：Leader 宕机 + 超过 Session Timeout

上面的表格里有一行很容易让人困惑：**"Leader 宕机 → 新 Leader 恢复 Session → 不删除"**。但现实中的故障往往是组合拳——Leader 挂了，客户端也连不上新 Leader，最终超时。这个组合场景下临时节点到底什么时候删、被谁删？

**场景复现**：

```
初始条件:
  - 5 节点集群, myid=1 是 Leader
  - 客户端直连 Leader (10.0.0.1:2181)
  - sessionTimeout = 12000ms
  - 已创建临时节点 /locks/mylock

故障: Leader-1 进程崩溃 (不仅是网络抖一下，是真的挂了)
  同时: 客户端所在机器也出现网络故障（或者客户端 Full GC 暂停 > 12s）
```
...
T=0ms              T=100ms          T=200ms         T=12000ms        T=15000ms
   │                   │                │                │               │
   ▼                   ▼                ▼                ▼               ▼
┌──────────┐    ┌──────────────┐  ┌───────────┐  ┌─────────────┐  ┌──────────┐
│Leader-1  │    │Follower-2,3,4│  │Node-2 当选 │  │新 Leader-2  │  │客户端网络 │
│宕机!     │    │发现 Leader   │  │新 Leader!  │  │判定:        │  │恢复, 终于 │
│进程崩溃  │    │心跳超时      │  │            │  │Session     │  │连上了     │
│          │    │(syncLimit)   │  │从快照恢复   │  │0x200...    │  │10.0.0.2   │
│          │    │             │  │SessionTracker│ │过期!       │  │          │
│          │    │各自进入      │  │            │  │            │  │ConnectReq │
│          │    │LOOKING 状态  │  │Session      │  │1.删除/locks│  │sessionId  │
│          │    │发起选举      │  │0x200...    │  │  /mylock   │  │+password │
│          │    │             │  │仍存活!      │  │2.触发Watcher│ │          │
│          │    │             │  │            │  │3.关闭Session│ │服务端响应:│
│          │    │             │  │            │  │            │  │Session   │
│          │    │             │  │            │  │            │  │Expired!  │
└──────────┘    └──────────────┘  └───────────┘  └─────────────┘  └──────────┘
     ...
     │               │                │                │               │
     ▼               ▼                ▼                ▼               ▼
  客户端 TCP     Follower 们      快照里有         sessionTimeout  客户端收到
  连接也断了     各自投自己        Session 记录     计时器到期      Expired 回调
  (因为连的      的票, PK         → 新 Leader     → 开始清理      → 必须重新
   就是Leader)   zxid/myid       知道这个 Session  → 临时节点       new ZK()!
                                  还活着            → 删除!
```
...
```
窗口 A: T=0ms → T=200ms (选举窗口)
  - 集群没有 Leader
  - 无法处理任何事务请求
  - 客户端的 Session 计时器在 tick（客户端本地计时 + 服务端 tickTime）
  - 但 Session 还没过期

窗口 B: T=200ms → T=12000ms (重连黄金窗口)
  - 新 Leader 已就位，SessionTracker 已从快照恢复
  - Leader 在等客户端重连
  - 只要客户端在此期间携带 sessionId+password 连上任意节点 → Session 恢复
  - 临时节点完好无损

窗口 C: T=12000ms+ (Session 过期后)
  - 新 Leader 判定 Session 过期
  - 执行清理：临时节点删除、Session 关闭、触发 Watcher
  - 此后客户端即使连上，也只能收到 Expired
```

**回答三个常见的追问**：

**追问 1：旧 Leader（myid=1）重启恢复后会怎样？**

```
旧 Leader-1 重启后:
  - 发现自己不在集群中 → 状态变为 LOOKING
  - 收到新 Leader-2 的心跳 → 发现 epoch 落后了
  - 以 Follower 身份加入集群 → 与新 Leader 进行数据同步 (DIFF/TRUNC)
  
  数据同步时:
  - 新 Leader 的 DataTree 中 /locks/mylock 早已被删除
  - 旧 Leader-1 本地可能还残留着 /locks/mylock 的数据
  - 同步（TRUNC 或 DIFF）后，旧 Leader-1 的本地数据被修正
  - → 临时节点不会再出现!
  
  结论: 旧 Leader 恢复后，临时节点不会"复活"。
       因为恢复的节点不是 Leader，必须与新 Leader 同步数据。
```

**追问 2：如果 sessionTimeout 设得很长（如 60s），Leader 宕机后新 Leader 会等满 60s 才删临时节点吗？**

```
会的。因为:

1. 新 Leader 从快照恢复 Session 时，该 Session 的过期时间是: 
   sessionTimeout - (当前时间 - Session 最后活跃时间)

2. 所以如果客户端在 Leader 宕机前 5s 还发过请求:
   - 剩余过期时间 = 60s - 5s = 55s
   - 新 Leader 会再等 55s 才判定 Session 过期

3. 这就是为什么 sessionTimeout 不建议设太长的原因之一:
   - Leader 宕机后，分布式锁要等 sessionTimeout 耗尽才释放
   - 太长 = 业务阻塞太久
   - 推荐 10s-20s
```

**追问 3：有没有可能 Leader 宕机导致临时节点被重复删除或遗漏删除？**

```
不会。原因:

1. 临时节点的归属信息存储在 DataTree 中 (ephemeralOwner 字段)
2. Session 过期时，Leader 调用 DataTree.killSession(sessionId, zxid)
   - 遍历 DataTree 中所有 ephemeralOwner == sessionId 的节点
   - 在同一个事务中批量删除
   - 这是一个原子操作

3. 如果旧 Leader 在 killSession Commit 之前宕机:
   - Proposal 可能已发但未 Commit
   - 新 Leader 会走 TRUNC 回退未 Commit 的 Proposal
   - 然后新 Leader 自己重新判定 Session 过期 → 重新执行 killSession
   
4. 因此不可能重复删除（zxid 唯一），也不可能遗漏（新 Leader 接管）
```
...
```
┌────────────────────────────────────────────────────────────────────┐
│  场景: Leader 宕机 + 超过 sessionTimeout                         │
│                                                                  │
│  临时节点删除时间点: sessionTimeout 耗尽的那一刻                   │
│  删除执行者:         当前在位的 Leader（新选举出的）              │
│  旧 Leader 恢复后:   临时节点不会复活（要和新 Leader 同步数据）    │
│                                                                  │
│  记住一条铁律:                                                    │
│  Session 过期 → 临时节点删除，跟 Leader 宕不宕机没关系            │
│  Leader 宕机只是让"判定 Session 过期"的职责转移给了新 Leader      │
│  但过期计时器从未停止，到了时间就一定删                           │
└────────────────────────────────────────────────────────────────────┘
```
