...
**Streamable HTTP** 是 2025-06-18 版本引入的统一远程传输，取代了早期版本里"HTTP + SSE 双端点"的割裂设计。它允许服务器用**响应流**下发长任务进度、日志等通知，同时支持请求-响应语义。2026-07-28 进一步演进：移除了会话头 `Mcp-Session-Id`，服务端主动通知改为通过 `subscriptions/listen` 订阅流，让协议在 HTTP 层做到**无状态、可水平扩展**（实现原理详见 3.2 节）。
...
### 3.2 Streamable HTTP 实现原理：内容协商、SSE 流与无状态

2.3 节把 stdio 和 Streamable HTTP 摆在一起对比了"是什么"，这一节把后者打开到 HTTP 层，看它到底怎么"流"起来。理解整个传输，先记住名字里的一个词——**Streamable（可流式）**：它不是"必须用流"，而是"**必要时可以用流**"。

**（1）核心机制：用 Accept 头做内容协商**

Streamable HTTP 最大的设计巧思，是把"要不要流"交给 HTTP 原生的**内容协商（Content Negotiation）**去表达，而不是在协议里另造一套开关。客户端发 POST 时带上：

```
Accept: application/json, text/event-stream
```

意思是"两种响应形态我都能处理"。服务器按需二选一：

| 场景 | 服务器选择 | 响应头 `Content-Type` |
|---|---|---|
| 一次就能算完（`tools/list`、`ping` 等） | 直接回一个 JSON-RPC 响应 | `application/json` |
| 长任务 / 要下发进度与日志 / 一次回多个消息 | 打开一个 SSE 响应流 | `text/event-stream` |

同一个端点、同一次 POST，响应形态由服务器动态决定——这就是 "Streamable" 三个字的全部含义：**请求是普通的，响应可以是流**。相比早期"HTTP + SSE 双端点"（客户端 POST 到一个 URL 下发请求、服务器从另一个 SSE URL 推送消息），Streamable HTTP 把它收敛成了**单端点**：一条 POST 通道同时承担"请求下发"与"响应回传"。

**（2）两种响应形态的线格式**

先看简单场景——`tools/list` 一次返回，走纯 JSON：
...
{"jsonrpc":"2.0","id":1,"result":{"tools":[{"name":"add","description":"求两个整数之和"}]}}
```

再看流式场景——一个长任务边执行边报进度。服务器把响应头换成 `text/event-stream`，用 SSE 事件逐条下发 JSON-RPC 消息，**最后一条才是带 `id` 的正式响应**：
...
event: message
data: {"jsonrpc":"2.0","method":"notifications/progress","params":{"progressToken":"t1","progress":0.4,"total":1.0}}

event: message
data: {"jsonrpc":"2.0","method":"notifications/progress","params":{"progressToken":"t1","progress":0.8,"total":1.0}}

event: message
data: {"jsonrpc":"2.0","id":1,"result":{"content":[{"type":"text","text":"执行完成"}]}}
```

三条规律记住即可：

1. 每个 SSE 事件固定是 `event: message` + `data: <一条完整 JSON-RPC 消息>`，事件名永远叫 `message`，真正内容在 `data` 里。
2. 一条流里可以装多类消息：**无 `id` 的是通知**（`notifications/progress`、`notifications/message` 等），**有 `id` 且与请求匹配的是正式响应**。
3. 服务器发完带 `id` 的响应后关闭流，客户端据此判断"这次调用结束"。

这背后复用的正是 3.1 节 JSON-RPC 的三类消息：请求-响应消息（有 `id`）保证调用有始有终，通知消息（无 `id`）顺路夹带进度与日志，两者在同一个 SSE 流里按到达顺序排列，客户端按 `id` 有无区分"这是结果还是过程"——这也是 3.4 节"请求作用域通知"的物理实现。

**（3）批量：一次 POST 多个请求**

JSON-RPC 2.0 支持批量（batch），Streamable HTTP 原样继承：请求体可以是一个 JSON 数组，响应（JSON 数组或 SSE 流里的多个 `message` 事件）按序对应。这对"启动时一口气拉全工具/资源/提示词清单"这类场景很有用，省掉 N 次往返：
...
**（4）会话的两代实现：Mcp-Session-Id → 无状态**

Streamable HTTP 自身也经历了从"有状态"到"无状态"的演进，这是 3.3 节"生命周期无状态化"在传输层的落点，这里先看它长什么样。

第一代（2025-06-18 起）是**会话式**的：服务器可以在响应里下发一个会话头，客户端后续请求都要带上：

```http
HTTP/1.1 200 OK
Mcp-Session-Id: 3f9c...a1
```

客户端下次请求带上 `Mcp-Session-Id: 3f9c...a1`，服务器据此找回会话上下文；同时客户端可另开一条 `GET /mcp`（`Accept: text/event-stream`）的 SSE 通道，专门接收服务器**主动发起**的消息（资源变更、日志推送等，它们不依附于某个具体请求）。代价是服务器要维护会话状态、网关要做会话粘滞，水平扩展困难。

第二代（2026-07-28 起）走向**无状态**：移除 `Mcp-Session-Id`，每个请求自描述（`_meta` 携带协议版本与客户端能力）；服务器**主动**消息不再靠独立的 GET 通道，改为客户端调用 `subscriptions/listen` 打开一条订阅流接收。于是任意请求都能打到任意实例，网关无需粘会话——"无状态、可水平扩展"从一句口号变成了 HTTP 层的现实。

**（5）与 stdio 的本质区别：只是"管道"不同**

把 stdio 和 Streamable HTTP 并排看，会发现两者承载的 JSON-RPC 消息**完全一样**——stdio 用换行分隔（每条消息一行），Streamable HTTP 用 HTTP body + 可能的 SSE 流。区别只在"管道"：一个走进程 stdin/stdout，一个走 TCP/HTTP。正因如此，MCP 的传输才是**可插拔**的：换传输不改业务逻辑，服务器只需换一个 TransportProvider（见 4.2 节的 `StdioServerTransportProvider` ↔ `HttpServletStreamableServerTransportProvider`），上面跑的协议语义一字不变。
...
两者经常**配合**使用：比如一个"写周报"的 SKILL 会写明"先收集本周 commit → 按模块归类 → 突出风险项 → 用固定模板输出"；其中"收集 commit"这一步，SKILL 会指示模型去调用 GitHub 的 MCP 工具。**SKILL 是剧本，MCP 是舞台上的道具与通道**——SKILL 教模型什么时候用、怎么用工具，MCP 负责把工具真正接进来。反过来，MCP 完全不依赖 SKILL：一个 MCP Server 没有 SKILL 也能被正常调用。

记忆口诀：MCP 让 AI **够得着**世界（工具、数据），SKILL 让 AI **知道怎么做**（流程、规范、经验）——一个管"手脚"，一个管"方法论"。
